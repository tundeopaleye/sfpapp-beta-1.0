...
// using a command bus. Basically making a post to the likes table assigning user_id and comment_id then redirect back
   ...
   // $this->execute(new StoryLikeCommand($user_id, $story_id));
  			...
  			$likeable_type1 = $request->get('likeable_type1');
			
			if($likeable_type1 == 'brand')
			{
				$likeableType = 'App\Brand';
				$likeable_id = $request->get('brand_id');
			}
			
			if($likeable_type1 == 'caption')
			{
				$likeableType = 'App\Caption';
				$likeable_id = $request->get('caption_id');
			}
			
			if($likeable_type1 == 'story')
			{
				$likeableType = 'App\Story';
			...
			'likeable_type' => $likeableType //temporary solution
